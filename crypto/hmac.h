...
/* Compute Hashed Message Authentication Code with SHA-224, over BUFFER
   data of BUFLEN bytes using the KEY of KEYLEN bytes, writing the
   output to pre-allocated 28 byte minimum RESBUF buffer.  Return 0 on
   success.  */
int
hmac_sha224 (const void *key, size_t keylen,
	   const void *in, size_t inlen, void *resbuf);
...
/* Compute Hashed Message Authentication Code with SHA-384, over BUFFER
   data of BUFLEN bytes using the KEY of KEYLEN bytes, writing the
   output to pre-allocated 48 byte minimum RESBUF buffer.  Return 0 on
   success.  */
int
hmac_sha384 (const void *key, size_t keylen,
	   const void *in, size_t inlen, void *resbuf);
